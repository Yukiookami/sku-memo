...
- 🔔 集成 `@capacitor/local-notifications`，支持 iOS/Android 本地推送通知
- ⏰ 任务可设置提醒时间，到期自动推送通知
- ⚡ 紧急提醒模式：可提前 5/10/15/30/60 分钟额外推送一条紧急通知
- 🔁 重复提醒模式（需开启紧急提醒）：从设置时刻起每 5 分钟推送一次，直至提醒时间前 5 分钟，最多 60 条
- 🗑️ 删除/完成任务时自动取消所有关联通知（主通知 + 紧急通知 + 重复通知）
- 🏷️ 任务卡片右下角显示提醒时间标签，已过期自动变灰并显示"已提醒"
- ✨ 添加/编辑面板内容为空时禁用提交按钮
